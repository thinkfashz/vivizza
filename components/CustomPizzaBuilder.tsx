    ...
    <div className="bg-[#1A1A1A] text-white rounded-[40px] p-8 md:p-12 shadow-2xl border border-white/5 relative overflow-hidden group">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-12">
        <div className="flex-1 space-y-10">
          <div>
            <span className="text-xs font-black text-[#A61D24] uppercase tracking-[0.4em] mb-3 block">Laboratorio de Sabor</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {DOUGH_TYPES.map((dough) => (
                <button
                  ...
                  className={`relative p-5 rounded-2xl border-2 transition-all text-left flex items-center justify-between group ${
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className={`text-[10px] ${selectedDoughId === dough.id ? 'text-stone-500' : 'text-stone-500'}`}>{dough.description}</p>
                  </div>
                ...
                <button
                  ...
                  className={`flex flex-col p-4 rounded-2xl border-2 transition-all ${
                    selectedIds.includes(ing.id) 
                      ? 'bg-[#A61D24] border-[#A61D24] scale-105 shadow-lg shadow-[#A61D24]/30' 
                      : 'bg-[#252525] border-white/5 hover:border-[#A61D24]/40'
                  }`}
                >
        ...
        <div className="lg:w-96">
          <div className="sticky top-32 bg-white rounded-3xl p-8 shadow-2xl text-stone-900 border border-stone-100">
            <h3 className="text-2xl font-bebas mb-6 border-b border-stone-100 pb-4 text-[#A61D24]">Resumen de Creación</h3>
            
            <div className="space-y-4 mb-8">
              <div className="flex justify-between items-center text-sm">
                <span className="text-stone-400 font-bold uppercase tracking-widest text-[10px]">Base Artesanal</span>
                <span className="font-bold text-stone-800">${CUSTOM_BASE_PRICE.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center text-sm p-3 bg-stone-50 rounded-xl">
                <div>
                  <p className="font-bold text-xs uppercase tracking-tight">Masa: {currentDough.name}</p>
                  <p className="text-[10px] text-stone-400">{currentDough.calories} kcal</p>
                </div>
                <span className="font-bold text-[#A61D24] text-xs">+{currentDough.price ? `$${currentDough.price.toLocaleString()}` : 'Gratis'}</span>
              </div>
              
              <div className="space-y-2">
                <p className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Ingredientes Extra</p>
                {selectedIds.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {selectedIds.map(id => {
                      const ing = AVAILABLE_INGREDIENTS.find(i => i.id === id);
                      return (
                        <span key={id} className="bg-red-50 text-[#A61D24] text-[10px] px-3 py-1.5 rounded-full font-black animate-fadeIn">
                          {ing?.name}
                        </span>
                      );
                    })}
                  ...
                  <p className="text-xs italic text-stone-300">Selecciona al menos un topping...</p>
                )}
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-stone-100">
              <div className="flex justify-between items-end mb-6">
                <span className="text-stone-400 font-bold uppercase tracking-widest text-[10px]">Precio Final</span>
                <span className="text-4xl font-bebas text-stone-900 leading-none">${totalPrice.toLocaleString('es-CL')}</span>
              </div>
              <button
                onClick={handleOrder}
                disabled={selectedIds.length === 0}
                className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all transform active:scale-95 shadow-xl ${
                  selectedIds.length > 0 
                    ? 'bg-[#A61D24] text-white hover:bg-[#8B181E] shadow-red-100 hover:shadow-red-200' 
