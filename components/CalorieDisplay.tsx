...
const CalorieDisplay: React.FC<CalorieDisplayProps> = ({ calories, label }) => {
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      else prevValue.current = endValue;
    };
    requestAnimationFrame(animate);
  }, [calories]);

  return (
    <div className="bg-white rounded-[32px] p-6 shadow-xl shadow-stone-200/50 border border-stone-100 flex flex-col gap-4 min-w-[280px]">
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[9px] uppercase font-black tracking-[0.2em] text-[#A61D24] mb-1">{label}</span>
          <div className="flex items-baseline gap-1">
            <span className="text-5xl font-bebas text-stone-900 leading-none">{displayValue.toLocaleString()}</span>
      ...
      <div className="w-full h-3 bg-stone-50 rounded-full overflow-hidden border border-stone-100 p-0.5">
        <div className="h-full bg-gradient-to-r from-yellow-400 via-[#A61D24] to-[#A61D24] rounded-full transition-all duration-700 ease-out progress-bar-glow" style={{ width: `${percentage}%` }}></div>
      </div>
    </div>
  );
};
